...
让用户**只用属性标记（Attribute）**就能为任意数据结构生成一套完整的 Inspector 渲染界面，布局灵活、代码简洁、运行时代码零污染。
...
不加任何 Attribute，任何 `[Serializable]` 类就能传入 Inspector 查看和编辑：
...
一句代码打开 Inspector——字段类型自动匹配编辑器控件：

```csharp
// 传入任意可序列化对象
var window = InspectorWindow.Open(playerConfig);
// 或挂载到 EditorWindow 中
GetHost().BuildDOM(new InspectorElement { Target = playerConfig });
```
- `int` → 整数输入框
- `float` → 浮点数输入框
- `string` → 文本框
...
系统自动感知 Inspector 之外的字段修改（动画驱动、网络回调、其他工具窗口等），并自动刷新显示：
...
Inspector 系统是 UniDecl 的**可选插件**，加载后自动启用。不加载时 UniDecl 核心正常工作，不受影响。
...
| 约束 | 说明 |
|------|------|
| Inspector 是可选插件 | 不加载模块时 UniDecl 核心正常工作 |
