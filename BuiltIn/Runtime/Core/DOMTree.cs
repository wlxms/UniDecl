...
namespace UniDecl.BuiltIn.Runtime.Core
{
    ...
    public class DOMTree
    {
        ...
        /// <summary>
        /// 推入 UndoScope——后续展开的子元素的 ElementState 将自动携带此 Scope。
        /// 用于 PropertyGridElement 等容器为其子 Widget 提供 Snapshot 作用域。
        /// 必须与 PopScope() 成对调用（建议用 try/finally）。
        /// </summary>
        public void PushScope(UndoScope scope)
        {
            if (scope != null)
        ...
        /// <summary>
        /// 弹出最近一次 PushScope 推入的 UndoScope。
        /// </summary>
